...
\todoin{
	\begin{itemize}
		\item invariants to show:
		      \begin{enumerate}
			      \item type variables are replaced exactly once during meet computation
			      \item substitution $\sigma$ is variable free
			      \item meet arguments draw from a finite set of expressions
			            (the original input, the finite number of variable substitutions, the right hand sides of type definitions in $S$)
		      \end{enumerate}
		\item fixes
		      \begin{itemize}
			      \item invariants 1 and 2 allow us to fix the size estimate: we can use the unknown but fixed size of the term that is subsituted for the variable as the size for the variable
		      \end{itemize}
	\end{itemize}}
\begin{proof}
	...
	Consequently, only finitely many expressions can occur as arguments of recursive calls.\todo[inline]{%
		...
		For example, the meet of $g(X,X,X)$  and $g(f(a|b),f(a|b),f(a|c))$ results in $\sigma(X)=f(a)$
		where $f(a)$ is a fresh subexpressions.}

	Since both $K$ and the set of possible recursive-call arguments are finite, only finitely many guard pairs $(t,e)$ can arise during the computation.
	...
	\begin{align*}
		|\top|=|t|=|v|                                  & =1,                     \\
		|f(e_1,\ldots,e_n)|                             & =1+\sum_{i=1}^{n}|e_i|, \\
		\bigl\lvert e_1 \mid \cdots \mid e_n\bigr\rvert & =1+\sum_{i=1}^{n}|e_i|.
	\end{align*}
